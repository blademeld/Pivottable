...
callWithJQuery ($) ->
    ...
    aggregatorTemplates =
#CHANGE HERE - added value aggregator
        value: (formatter=usFmtInt) -> () -> (data, rowKey, colKey) ->
            count: 0
            push: -> this.count
            value: -> colKey
            format: (value) -> value
    ...
    renderers =
        "테이블":          (pvtData, opts) ->   pivotTableRenderer(pvtData, opts)
...
#                selectNone: "Select None"
    ...
    $.fn.pivotUI = (input, inputOpts, overwrite = false, locale="en") ->
        ...
        try
            ...
            #renderer control
            rendererControl = $("<td>").attr('id', 'topleft')
...
#CHANGE HERE - hide item display
                    valueList.append $('<div>').append($("<h4>").text("종류")).attr('id', 'popupHeader')
...
#CHANGE HERE - en to kr
#CHANGE HERE - added button id 'popupOK'
                    $("<p>").appendTo(valueList)
                        .append $("<button>", {type:"button"}).attr('id', 'popupOK').text("확인").bind "click", updateFilter
            ...
            $("<td>").addClass('pvtVals')
              .appendTo(tr1)
#CHANGE HERE - uncomment this to reapply the aggregator              .append(aggregator)
              .append($("<br>"))
